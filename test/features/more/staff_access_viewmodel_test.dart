...
void main() {
  ...
  test('redirects non owners and loads staff for owners', () async {
    ...
    when(
      () => repository.fetchStaff('t'),
    ).thenAnswer((_) async => Success([staff()]));
    await model.init();
  ...
  test('loads failure and formats access packs', () async {
    ...
    when(
      () => repository.fetchStaff('t'),
    ).thenAnswer((_) async => const Failure(ValidationFailure('failed')));
    final model = StaffAccessViewModel(repository);
    await model.init();
  ...
  test('invites staff on success and reports failure', () async {
    ...
    when(
      () => sheets.showCustomSheet<StaffMember, dynamic>(
        variant: any(named: 'variant'),
        isScrollControlled: any(named: 'isScrollControlled'),
      ),
    ).thenAnswer((_) async => SheetResponse(data: invitee));
    when(
      () => repository.inviteStaff(invitee),
    ).thenAnswer((_) async => Success(invitee));
    final model = StaffAccessViewModel(repository);
    await model.invite();

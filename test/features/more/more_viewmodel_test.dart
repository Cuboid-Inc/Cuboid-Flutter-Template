...
void main() {
  ...
  setUp(() {
    PackageInfo.setMockInitialValues(
      appName: 'FleetGo',
      packageName: 'com.cuboidinc.fleetgo',
      version: '0.1.0',
      buildNumber: '1',
      buildSignature: '',
    );
    ...
    when(
      () => navigation.clearStackAndShow(any()),
    ).thenAnswer((_) async => null);
  });
  tearDown(locator.reset);

  test(
    'loads counts, exposes permissions, refreshes, and opens routes',
    () async {
      when(() => auth.currentAccess).thenReturn(access());
      when(
        () => repository.fetchMenuCounts(),
      ).thenAnswer((_) async => Success(counts()));

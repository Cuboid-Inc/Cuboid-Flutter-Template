...
Agreement agreement({String id = 'a', RateModel model = RateModel.monthly}) => Agreement(
  id: id, reference: 'R', name: 'Monthly', customerId: 'c', rateModel: model,
  baseRate: 1000, overtimeRate: 20, extraDayRate: 30, extraTripRate: 40,
  defaultExtras: {'Parking': 5}, defaultVehicleId: 'v',
);
...
void main() {
  ...
  test('loads monthly agreements and manages charges', () async {
    ...
    final model = MonthlyWorkViewModel(agreementRepository: agreements, workRepository: work);
    await model.init();
    ...
    model.addCharge(MonthlyExtraType.custom, quantity: 2, rate: 12.345, customName: ' Extra ');
  ...
  test('validates steps and saves a work order', () async {
    final selected = agreement();
    when(() => agreements.fetchAgreements()).thenAnswer((_) async => Success([selected]));
    final created = WorkOrder(
      id: 'w', number: 'WO-1', customerId: 'c', agreementId: 'a',
      date: DateTime(2026, 7, 1), pickup: 'Monthly hire', destination: 'Monthly',
    );
    when(() => work.create(any())).thenAnswer((_) async => Success(created));
    final model = MonthlyWorkViewModel(agreementRepository: agreements, workRepository: work);
    await model.init();
    await model.next();
    verify(() => snackbar.showCustomSnackBar(message: 'Please select an agreement', variant: SnackbarType.warning)).called(1);
    model.selectAgreement(selected);
    await model.next();
    expect(model.step, 2);
    model.removeCharge(0);
    await model.next();
    verify(() => snackbar.showCustomSnackBar(message: 'Add at least one charge', variant: SnackbarType.warning)).called(1);
    ...
    verify(() => snackbar.showCustomSnackBar(message: 'WO-1 created', variant: SnackbarType.success)).called(1);
    model.step = 2;
    model.back();
    expect(model.step, 1);
    model.back();
  ...
  test('handles agreement page and save failures', () async {
    when(() => agreements.fetchAgreementsPage(
      pageNumber: 1,
      pageSize: 50,
      search: null,
      rateModel: RateModel.monthly,
    ...
    when(() => work.create(any())).thenAnswer(
      (_) async => const Failure(ValidationFailure('save failed')),
    );
    model.step = 3;
    await model.next();
    verify(() => snackbar.showCustomSnackBar(message: 'save failed', variant: SnackbarType.error)).called(1);
  });
}

...
class DriverRepository with RepositoryCacheMixin {
  DriverRepository([AuthRepository? auth])
  ...
  Future<Result<List<Driver>>> fetchDrivers() => cached(
    _cache,
    'all',
    () => guard(() async {
    ...
    return cached(
      ...
      () => guard(() async {
        ...
        return PaginatedResult(
          items: response.data.map(DriverRow.fromRow).toList(),
  ...
  Future<Result<Driver>> addDriver(Driver driver) async {
    final result = await guard(() async {
      final tenantId = _requireTenantId();
      final row = driver.toRow(tenantId);
      final saved = driver.id.isEmpty
          ...
          : await Supabase.instance.client
                ...
                .select()
                .single();
      return DriverRow.fromRow(saved);
    });
    if (result case Success()) _onMutated();
    return result;
  }

  Future<Result<void>> archiveDriver(String id) async {
    final result = await guard(() async {
      _requireTenantId();
      await Supabase.instance.client
          .from('drivers')
  ...
  /// Driver counts feed into MoreRepository's menu badge, so a mutation

...
void main() {
  test('creates the supabase example repository, model, and migration', () async {
  ...
  test('rejects symlink ancestors for lib/supabase', () async {
    ...
    File('${targetLib.path}/app/app.locator.dart')
      ..parent.createSync(recursive: true)
  ...
  test('rolls back created files and restores app.locator.dart when the '
      'migration write fails', () async {
    final root = _projectRoot();
    addTearDown(() => root.deleteSync(recursive: true));
    final beforeFiles = _relativeFiles(root);
    final beforeApp = _appFile(root).readAsStringSync();
  ...
  test('rolls back created files and leaves app.locator.dart untouched when '
      'the locator update fails', () async {
    final root = _projectRoot();
    addTearDown(() {
